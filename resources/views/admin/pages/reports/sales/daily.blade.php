        ...
        <div class="container-fluid p-0">
            <div class="users-premium">
                ...
                <div class="users-header">
                    <h5 class="users-page-title">تقرير المبيعات اليومي</h5>
                    <a href="{{ route('admin.reports.sales.daily', array_merge(request()->only(['date', 'branch_id']), ['format' => 'csv'])) }}"
                        class="users-btn-secondary" id="sales-daily-export">
                        <i class="fas fa-file-csv"></i> تصدير CSV
                    </a>
                </div>

                <div class="users-filters-card">
                    <form id="sales-daily-filters" action="{{ route('admin.reports.sales.daily') }}" method="GET" class="users-filters-form">
                        ...
                        <button type="button" id="sales-daily-clear" class="users-btn-filter users-btn-filter--clear">
                            <i class="fas fa-times me-1"></i> مسح
                        </button>
                    </form>
                </div>

                <div id="sales-daily-card" class="users-table-card--loading-target" style="background: transparent; border: none; box-shadow: none; padding: 0;">
                    <div id="sales-daily-summary">
                        @include('admin.pages.reports.sales.partials.daily-summary')
                    </div>
                </div>
...
@section('script')
    ...
    <script>
        (function () {
            var filtersForm = document.getElementById('sales-daily-filters');
            var summaryEl = document.getElementById('sales-daily-summary');
            var reportCard = document.getElementById('sales-daily-card');
            var clearBtn = document.getElementById('sales-daily-clear');
            var exportLink = document.getElementById('sales-daily-export');
            var isLoading = false;
