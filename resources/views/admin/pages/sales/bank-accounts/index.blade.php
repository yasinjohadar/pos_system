                                ...
                                <tbody>
                                    @forelse($bankAccounts as $ba)
                                        <tr>
                                            ...
                                            <td>{{ number_format((float)($ba->opening_balance ?? 0), 2) }}</td>
                                            <td>{{ $ba->currency ?? '—' }}</td>
                                            <td>
                                                @if($ba->is_active)
                                            ...
                                            <td>
                                                <div class="d-flex gap-2 justify-content-center flex-wrap">
                                                    ...
                                                    <a href="{{ route('admin.bank-accounts.edit', $ba) }}" class="btn btn-sm btn-primary" title="تعديل"><i class="fas fa-edit"></i></a>
                                                    @endcan
                                                    @can('bank-account-delete')
                                                    <form action="{{ route('admin.bank-accounts.destroy', $ba) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                                                        @csrf
                                                        @method('DELETE')
