    ...
    <tr>
        ...
        <td>
            <div class="users-user-cell">
                ...
                <span class="users-user-name" style="cursor: default;">{{ $inv->customer->name ?? '—' }}</span>
            </div>
        </td>
        <td>
                ...
                @can('sale-invoice-edit')
                    @if ($inv->status === 'draft')
                        <a class="users-action-btn users-action-btn--edit"
                            href="{{ route('admin.sale-invoices.edit', $inv) }}"
                ...
                @can('sale-invoice-delete')
                    @if ($inv->status === 'draft')
                        <button type="button" class="users-action-btn users-action-btn--delete"
                            ...
                            data-delete-action="{{ route('admin.sale-invoices.destroy', $inv) }}"
...
@empty
    <tr>
        <td colspan="9" class="users-empty">لا توجد فواتير — شغّل SalesSeeder أو أنشئ فاتورة جديدة</td>
    </tr>
@endforelse
