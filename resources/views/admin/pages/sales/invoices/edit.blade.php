...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            <div class="my-auto">
                <h5 class="page-title fs-21 mb-1">تعديل فاتورة: {{ $saleInvoice->number }}</h5>
            </div>
            <div>
                <a href="{{ route('admin.sale-invoices.show', $saleInvoice) }}" class="btn btn-secondary btn-sm">
        ...
        <form action="{{ route('admin.sale-invoices.update', $saleInvoice) }}" method="POST" id="invoice-form">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-lg-8">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="branch_id" id="branch_id" class="form-select" required>
                                        @foreach($branches as $b)
                                            <option value="{{ $b->id }}" {{ old('branch_id', $saleInvoice->branch_id) == $b->id ? 'selected' : '' }}>{{ $b->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <select name="customer_id" class="form-select">
                                        ...
                                        @foreach($customers as $c)
                                            <option value="{{ $c->id }}" {{ old('customer_id', $saleInvoice->customer_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <select name="warehouse_id" id="warehouse_id" class="form-select" required>
                                        @foreach($warehouses as $w)
                                            <option value="{{ $w->id }}" data-branch="{{ $w->branch_id }}" {{ old('warehouse_id', $saleInvoice->warehouse_id) == $w->id ? 'selected' : '' }}>{{ $w->name }} ({{ $w->branch->name ?? '' }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">تاريخ الفاتورة <span class="text-danger">*</span></label>
                                    <input type="date" name="invoice_date" class="form-control" value="{{ old('invoice_date', $saleInvoice->invoice_date->format('Y-m-d')) }}" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">نسبة الضريبة %</label>
                                    <input type="number" step="0.01" name="tax_rate" id="tax_rate" class="form-control" value="{{ old('tax_rate', $saleInvoice->tax_rate) }}" min="0" max="100">
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">نوع الخصم</label>
                                    <select name="discount_type" id="discount_type" class="form-select">
                                        <option value="">بدون</option>
                                        <option value="fixed" {{ old('discount_type', $saleInvoice->discount_type) === 'fixed' ? 'selected' : '' }}>مبلغ</option>
                                        <option value="percent" {{ old('discount_type', $saleInvoice->discount_type) === 'percent' ? 'selected' : '' }}>نسبة %</option>
                                    </select>
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">قيمة الخصم</label>
                                    <input type="number" step="0.01" name="discount_value" id="discount_value" class="form-control" value="{{ old('discount_value', $saleInvoice->discount_value) }}" min="0">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">كود الخصم (كوبون)</label>
                                    <input type="text" name="coupon_code" class="form-control" value="{{ old('coupon_code', $saleInvoice->coupon?->code) }}" placeholder="أدخل كود الكوبون ثم احفظ الفاتورة">
                                </div>
                                <div class="col-12">
                                    <label class="form-label">ملاحظات</label>
                                    <textarea name="notes" class="form-control" rows="1">{{ old('notes', $saleInvoice->notes) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered mb-0" id="items-table">
                                    ...
                                    <tbody id="items-tbody">
                                        @php $items = old('items', $saleInvoice->items->map(fn($i) => ['product_id' => $i->product_id, 'quantity' => $i->quantity, 'unit_price' => $i->unit_price, 'warehouse_id' => $i->warehouse_id])->toArray()); @endphp
            ...
            <div class="d-flex gap-2 mb-4">
                ...
                <a href="{{ route('admin.sale-invoices.show', $saleInvoice) }}" class="btn btn-secondary">إلغاء</a>
            </div>
        </form>
    </div>
</div>
