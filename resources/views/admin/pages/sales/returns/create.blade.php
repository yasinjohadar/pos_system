...
<div class="main-content app-content">
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            ...
            <div>
                <a href="{{ $saleInvoice ? route('admin.sale-invoices.show', $saleInvoice) : route('admin.sale-returns.index') }}" class="btn btn-secondary btn-sm">
        ...
        <form action="{{ route('admin.sale-returns.store') }}" method="POST">
            @csrf
            @if($saleInvoice)
            <input type="hidden" name="sale_invoice_id" value="{{ $saleInvoice->id }}">
            @endif
            @if(!$saleInvoice)
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-body">
                    <label class="form-label">الفاتورة الأصلية <span class="text-danger">*</span></label>
                    <select name="sale_invoice_id" id="sale_invoice_id" class="form-select @error('sale_invoice_id') is-invalid @enderror" required>
                        <option value="">اختر الفاتورة</option>
                        @foreach($invoices ?? [] as $inv)
                            <option value="{{ $inv->id }}" {{ old('sale_invoice_id') == $inv->id ? 'selected' : '' }}>{{ $inv->number }} - {{ $inv->branch->name ?? '' }} - {{ $inv->invoice_date->format('Y-m-d') }}</option>
                        @endforeach
                    </select>
                    @error('sale_invoice_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    <p class="text-muted small mt-1">أو أنشئ المرتجع من صفحة الفاتورة (زر "مرتجع").</p>
                </div>
            </div>
            @endif
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="warehouse_id" class="form-select @error('warehouse_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($warehouses ?? [] as $w)
                                            <option value="{{ $w->id }}" {{ old('warehouse_id', $saleInvoice->warehouse_id ?? '') == $w->id ? 'selected' : '' }}>{{ $w->name }} ({{ $w->branch->name ?? '' }})</option>
                                        @endforeach
                                    </select>
                                        ...
                                        @if($saleInvoice)
                                            ...
                                            <tr class="return-item-row">
                                                <td>
                                                    ...
                                                    {{ $item->product->name ?? '—' }}
                                                    <small class="text-muted">(الكمية القابلة للمرتجع: {{ $item->quantity_remaining }})</small>
                                                </td>
...
@if(!$saleInvoice && isset($invoices))
@push('scripts')
<script>
document.addEventListener('DOMContentLoaded', function() {
    var sel = document.getElementById('sale_invoice_id');
    if (sel) {
        sel.addEventListener('change', function() {
            var id = this.value;
            if (id) {
                window.location = '{{ route("admin.sale-returns.create") }}?sale_invoice_id=' + id;
            }
        });
    }
});
