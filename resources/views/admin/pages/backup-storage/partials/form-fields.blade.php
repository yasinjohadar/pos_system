@php
    ...
    $storageConfig = ($config ?? null) instanceof BackupStorageConfig ? $config : null;
    $drivers = $drivers ?? [];
@endphp
    ...
    <div class="users-form-grid">
        <div class="users-form-group users-form-group--full">
            ...
            <input type="text" class="users-form-input @error('name') is-invalid @enderror"
                id="name" name="name" value="{{ old('name', $storageConfig?->name) }}" required>
            @error('name')
        ...
        <div class="users-form-group users-form-group--full">
            ...
            <select class="users-form-select @error('driver') is-invalid @enderror" id="driver" name="driver" required>
                ...
                @foreach ($drivers as $key => $label)
                    <option value="{{ $key }}" {{ old('driver', $storageConfig?->driver) == $key ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
            @error('driver')
                <div class="users-form-error">{{ $message }}</div>
    ...
    <div class="users-form-grid">
        <div class="users-form-group">
            ...
            <input type="number" class="users-form-input" id="priority" name="priority"
                value="{{ old('priority', $storageConfig?->priority ?? 0) }}" min="0">
            <span class="users-form-hint">الأعلى يُستخدم أولاً</span>
        </div>

        <div class="users-form-group">
            <label for="max_backups" class="users-form-label"><i class="fas fa-copy"></i> الحد الأقصى للنسخ</label>
            <input type="number" class="users-form-input" id="max_backups" name="max_backups"
                value="{{ old('max_backups', $storageConfig?->max_backups) }}" min="1">
            <span class="users-form-hint">اتركه فارغاً لعدم تحديد حد</span>
        </div>
            ...
            <div class="users-form-switches">
                <div class="users-form-switch">
                    ...
                    <div class="users-form-switch__info">
                        ...
                        <span class="users-form-switch__desc">أماكن التخزين غير النشطة لا تُستخدم للنسخ الاحتياطي</span>
                    </div>
                    <label class="users-toggle users-toggle--compact">
                        <input type="checkbox" class="users-toggle-input users-form-toggle-input" id="is_active"
                            ...
                            {{ old('is_active', $storageConfig?->is_active ?? true) ? 'checked' : '' }}>
                        <span class="users-toggle-track"><span class="users-toggle-thumb"></span></span>
                        <span class="users-toggle-label">{{ old('is_active', $storageConfig?->is_active ?? true) ? 'نشط' : 'غير نشط' }}</span>
                    </label>
                </div>
                <div class="users-form-switch">
                    ...
                    <div class="users-form-switch__info">
                        ...
                        <span class="users-form-switch__desc">Redundancy — نسخ احتياطية على أكثر من مزود</span>
                    </div>
                    <label class="users-toggle users-toggle--compact">
                        <input type="checkbox" class="users-toggle-input users-form-toggle-input" id="redundancy"
                            ...
                            {{ old('redundancy', $storageConfig?->redundancy) ? 'checked' : '' }}>
                        <span class="users-toggle-track"><span class="users-toggle-thumb"></span></span>
                        <span class="users-toggle-label">{{ old('redundancy', $storageConfig?->redundancy) ? 'مفعّل' : 'معطّل' }}</span>
                    </label>
                </div>
            </div>
        </div>
    ...
    <div class="users-form-grid">
        <div class="users-form-group">
            ...
            <input type="number" step="0.01" class="users-form-input" name="pricing_config[storage_cost_per_gb]"
                value="{{ old('pricing_config.storage_cost_per_gb', $storageConfig?->pricing_config['storage_cost_per_gb'] ?? '') }}">
        </div>
        <div class="users-form-group">
            ...
            <input type="number" step="0.01" class="users-form-input" name="pricing_config[upload_cost_per_gb]"
                value="{{ old('pricing_config.upload_cost_per_gb', $storageConfig?->pricing_config['upload_cost_per_gb'] ?? '') }}">
        </div>
        <div class="users-form-group">
            ...
            <input type="number" step="0.01" class="users-form-input" name="pricing_config[download_cost_per_gb]"
                value="{{ old('pricing_config.download_cost_per_gb', $storageConfig?->pricing_config['download_cost_per_gb'] ?? '') }}">
        </div>
        <div class="users-form-group">
            ...
            <input type="number" step="0.01" class="users-form-input" name="monthly_budget"
                value="{{ old('monthly_budget', $storageConfig?->monthly_budget) }}">
        </div>
        <div class="users-form-group">
            ...
            <input type="number" step="0.01" class="users-form-input" name="cost_alert_threshold"
                value="{{ old('cost_alert_threshold', $storageConfig?->cost_alert_threshold) }}">
        </div>
    </div>
</div>
