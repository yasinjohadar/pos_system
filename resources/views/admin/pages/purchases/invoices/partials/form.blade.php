@php
    ...
    $oldItems = old('items');
    if ($oldItems === null && $purchaseInvoice) {
        $oldItems = $purchaseInvoice->items->map(fn ($item) => [
...
<form action="{{ $action }}" method="POST" id="invoice-form"
    data-product-price-url="{{ route('admin.purchase-invoices.product-cost') }}"
    ...
    <div class="users-form-layout">
        @include('admin.components.premium.form-aside', [
            ...
            'title' => $purchaseInvoice ? 'تعديل فاتورة شراء' : 'فاتورة شراء جديدة',
            'text' => 'أنشئ فاتورة مسودة ثم اعتمدها لإدخال المخزون. ابحث عن المورد والمنتجات مباشرة دون تحميل القوائم كاملة.',
            'tips' => ['الفاتورة تُحفظ كمسودة حتى الاعتماد', 'يُجلب سعر التكلفة تلقائياً عند اختيار المنتج', 'يمكن تسجيل الدفعات بعد الاعتماد'],
        ])
        ...
        <div class="users-form-card">
            ...
            <div class="users-form-card__body">
                <div class="users-form-grid">
                    <div class="users-form-group">
                        ...
                        <select name="branch_id" id="branch_id" class="users-form-select @error('branch_id') is-invalid @enderror" required>
                            ...
                            @foreach ($branches as $b)
                                <option value="{{ $b->id }}" {{ old('branch_id', optional($purchaseInvoice)->branch_id) == $b->id ? 'selected' : '' }}>{{ $b->name }}</option>
                            @endforeach
                        </select>
                        @error('branch_id')
                    ...
                    <div class="users-form-group">
                        <label for="supplier_id" class="users-form-label"><i class="fas fa-truck"></i> المورد</label>
                        @include('admin.components.premium.supplier-select', [
                            'selected' => $selectedSupplier ?? null,
                            'required' => false,
                            'placeholder' => 'بدون مورد / ابحث بالاسم...',
                        ])
                        @error('supplier_id')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="users-form-group">
                        ...
                        <select name="warehouse_id" id="warehouse_id" class="users-form-select @error('warehouse_id') is-invalid @enderror" required>
                            ...
                            @foreach ($warehouses as $w)
                                <option value="{{ $w->id }}" data-branch="{{ $w->branch_id }}" {{ old('warehouse_id', optional($purchaseInvoice)->warehouse_id) == $w->id ? 'selected' : '' }}>
                    ...
                    <div class="users-form-group">
                        ...
                        <input type="date" name="invoice_date" id="invoice_date" class="users-form-input @error('invoice_date') is-invalid @enderror"
                            value="{{ old('invoice_date', optional($purchaseInvoice)->invoice_date?->format('Y-m-d') ?? date('Y-m-d')) }}" required>
                        @error('invoice_date')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="users-form-group">
                        ...
                        <input type="number" step="0.01" name="tax_rate" id="tax_rate" class="users-form-input @error('tax_rate') is-invalid @enderror"
                            value="{{ old('tax_rate', optional($purchaseInvoice)->tax_rate ?? 0) }}" min="0" max="100">
                        @error('tax_rate')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="users-form-group">
                        ...
                        <select name="discount_type" id="discount_type" class="users-form-select">
                            ...
                            <option value="fixed" {{ old('discount_type', optional($purchaseInvoice)->discount_type) === 'fixed' ? 'selected' : '' }}>مبلغ</option>
                            <option value="percent" {{ old('discount_type', optional($purchaseInvoice)->discount_type) === 'percent' ? 'selected' : '' }}>نسبة %</option>
                        </select>
                    </div>
                    ...
                    <div class="users-form-group">
                        ...
                        <input type="number" step="0.01" name="discount_value" id="discount_value" class="users-form-input"
                            value="{{ old('discount_value', optional($purchaseInvoice)->discount_value ?? 0) }}" min="0">
                    </div>
                    ...
                    <div class="users-form-group users-form-group--full">
                        ...
                        <textarea name="notes" id="notes" class="users-form-textarea" rows="2">{{ old('notes', optional($purchaseInvoice)->notes) }}</textarea>
                    </div>
                </div>
                        ...
                        <table class="users-table" id="items-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 130px;">سعر التكلفة</th>
