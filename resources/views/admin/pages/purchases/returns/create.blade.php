        ...
        <div class="container-fluid p-0">
            <div class="users-premium">
                ...
                <div class="users-header">
                    ...
                    <a href="{{ $purchaseInvoice ? route('admin.purchase-invoices.show', $purchaseInvoice) : route('admin.purchase-returns.index') }}" class="users-btn-secondary">
                ...
                <form action="{{ route('admin.purchase-returns.store') }}" method="POST" id="purchase-return-form"
                    data-invoice-data-url="{{ url('admin/purchase-returns/invoice-data') }}">
                    @csrf

                    <div class="users-form-layout">
                        @include('admin.components.premium.form-aside', [
                            'icon' => 'fa-undo',
                            'title' => 'تسجيل مرتجع شراء',
                            'text' => 'اختر الفاتورة المعتمدة ثم حدد الكميات المرتجعة. يُحفظ المرتجع بحالة انتظار حتى تكمله لصرف الكميات من المخزون.',
                        ...
                        <div class="users-form-card">
                            ...
                            <div class="users-form-card__body">
                                @if ($purchaseInvoice)
                                    <input type="hidden" name="purchase_invoice_id" id="purchase_invoice_id" value="{{ $purchaseInvoice->id }}">
                                    <div class="users-form-group users-form-group--full">
                                        ...
                                        <div class="users-badge users-badge--role" style="font-size: 0.875rem; padding: 0.5rem 0.75rem;">
                                            {{ $purchaseInvoice->number }} — {{ $purchaseInvoice->branch->name ?? '—' }} — {{ $purchaseInvoice->invoice_date->format('Y-m-d') }}
                                        </div>
                                    </div>
                                @else
                                    <div class="users-form-group users-form-group--full">
                                        <label for="purchase_invoice_id" class="users-form-label"><i class="fas fa-receipt"></i> الفاتورة الأصلية <span class="users-form-required">*</span></label>
                                        <select name="purchase_invoice_id" id="purchase_invoice_id" class="users-form-select users-invoice-search" required data-placeholder="ابحث برقم الفاتورة...">
                                            <option value=""></option>
                                            @if (old('purchase_invoice_id'))
                                                @php $oldInv = \App\Models\PurchaseInvoice::with('branch')->find(old('purchase_invoice_id')); @endphp
                                                @if ($oldInv)
                                    ...
                                    <div class="users-form-group">
                                        ...
                                        <select name="warehouse_id" id="warehouse_id" class="users-form-select" required>
                                            ...
                                            @foreach ($warehouses as $w)
                                                <option value="{{ $w->id }}" {{ old('warehouse_id', $purchaseInvoice->warehouse_id ?? '') == $w->id ? 'selected' : '' }}>
                                ...
                                <div class="users-form-actions" id="return-form-actions" style="{{ ($purchaseInvoice && $purchaseInvoice->items->isNotEmpty()) ? '' : 'display:none;' }}">
                                    <button type="submit" class="users-btn-submit"><i class="fas fa-save"></i> حفظ المرتجع</button>
                                    <a href="{{ $purchaseInvoice ? route('admin.purchase-invoices.show', $purchaseInvoice) : route('admin.purchase-returns.index') }}" class="users-btn-secondary">إلغاء</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('script')
    ...
    <script>
        (function () {
            ...
            var preloadedInvoiceId = @json($purchaseInvoice?->id);
            ...
            function renderItems(items) {
                ...
                items.forEach(function (item) {
                    ...
                    tr.innerHTML =
                        '<td>' +
                            '<input type="hidden" name="items[' + i + '][purchase_invoice_item_id]" value="' + item.purchase_invoice_item_id + '">' +
                            '<input type="hidden" name="items[' + i + '][product_id]" value="' + item.product_id + '">' +
                            '<div class="users-user-cell">' +
                                '<div class="users-avatar"><i class="fas fa-box"></i></div>' +
                                '<div>' +
            ...
            var invoiceSelect = document.getElementById('purchase_invoice_id');
            if (invoiceSelect && invoiceSelect.tagName === 'SELECT') {
                AdminPremium.initProductSearch({
                    url: '{{ route('admin.purchase-returns.search-invoices') }}',
                    selector: '#purchase_invoice_id',
