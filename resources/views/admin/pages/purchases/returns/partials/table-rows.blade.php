@forelse ($returns as $ret)
    <tr>
        ...
        <td>
            <a href="{{ route('admin.purchase-invoices.show', $ret->purchase_invoice_id) }}" class="users-user-name">
                {{ $ret->purchaseInvoice->number ?? $ret->purchase_invoice_id }}
            </a>
        </td>
            ...
            <div class="users-actions">
                @can('purchase-return-show')
                    <a class="users-action-btn users-action-btn--view"
                        href="{{ route('admin.purchase-returns.show', $ret) }}"
                        title="عرض المرتجع">
...
@empty
    <tr>
        <td colspan="8" class="users-empty">لا توجد مرتجعات — شغّل PurchaseSeeder أو أنشئ مرتجعاً جديداً</td>
    </tr>
@endforelse
