(function (window) {
    ...
    function buildEditTemplates(currentConfig) {
        ...
        return {
            local: '<div class="mb-3"><label class="form-label">المسار (اختياري)</label><input type="text" class="form-control" name="config[path]" value="' + val('path', 'backups') + '"></div>',
            s3: '<div class="mb-3"><label class="form-label">Access Key ID <span class="text-danger">*</span></label><input type="text" class="form-control" name="config[access_key_id]" value="' + val('access_key_id') + '" required></div>' +
                ...
                '<div class="mb-3"><label class="form-label">Region <span class="text-danger">*</span></label><input type="text" class="form-control" name="config[region]" value="' + val('region', 'us-east-1') + '" required placeholder="مثال: us-east-1, eu-west-1"></div>' +
            ...
            ftp: '<div class="mb-3"><label class="form-label">Protocol</label><select class="form-select" name="config[protocol]"><option value="ftp" ' + selected('protocol', 'ftp') + '>FTP</option><option value="sftp" ' + selected('protocol', 'sftp') + '>SFTP</option></select></div>' +
                ...
                '<div class="mb-3"><label class="form-label">Root Path</label><input type="text" class="form-control" name="config[root]" value="' + val('root', '/backups') + '"></div>' +
    ...
    function init() {
        ...
        } else if (form.hasAttribute('data-old-config')) {
            try {
                oldConfig = JSON.parse(form.getAttribute('data-old-config') || '{}');
            } catch (error) {
                oldConfig = {};
            }
        }
        ...
        function fillOldValues() {
            if (isEdit || !Object.keys(oldConfig).length) {
                return;
            }
    ...
    window.BackupStorageForm = { init: init };
})(window);
